...
public class DataBase {
    ...
    private ArrayList<Game> downloadGames(ResultSet resultSet) throws SQLException {
        ...
        while(resultSet.next()){
            ...
            User user1 = new User();
            user1.setId(resultSet.getInt("User1"));
            game.setUser1(user1);
            User user2 = new User();
            user2.setId(resultSet.getInt("User2"));
            game.setUser2(user2);
            games.add(game);
        }
        setUsers(games);
    ...
    private void setUsers(ArrayList<Game> games) throws SQLException {
        for(Game g: games){
            ResultSet res1 = statement.executeQuery("SELECT * from Users where id =  " + g.user1.id + ";");
            User user = new User();
            while(res1.next()) {
                user.setId(res1.getInt("id"));
                user.setLogin(res1.getString("login"));
            }
            g.setUser1(user);
            res1.close();
        }
        for(Game g: games){
            ResultSet res1 = statement.executeQuery("SELECT * from Users where id =  " + g.user2.id + ";");
            User user = new User();
            while(res1.next()) {
                user.setId(res1.getInt("id"));
                user.setLogin(res1.getString("login"));
    ...
    public String getMoves(int game, int user){
        String moves = null;
        try {
            connect();
            resultSet = null;
    ...
    private String createMoves(ResultSet resultSet) throws SQLException {
        String moves = new String();
        while (resultSet.next()) {
            moves = resultSet.getString("moves");
        }
